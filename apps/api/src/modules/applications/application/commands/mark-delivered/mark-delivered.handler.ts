...
export class MarkDeliveredHandler implements ICommandHandler<MarkDeliveredCommand> {
  ...
  async execute(command: MarkDeliveredCommand): Promise<ApplicationReadModel> {
    ...
    const actor = Actor.system('delivery-tracking-webhook');
    ...
    try {
      ...
      throw new BadRequestException(error instanceof Error ? error.message : 'Invalid delivery evidence');
    }

    try {
      application.markDelivered(actor, correlationId, evidence, confidence);
    } catch (error) {
