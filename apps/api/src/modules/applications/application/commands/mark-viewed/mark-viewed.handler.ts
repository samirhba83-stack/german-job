...
export class MarkViewedHandler implements ICommandHandler<MarkViewedCommand> {
  ...
  async execute(command: MarkViewedCommand): Promise<ApplicationReadModel> {
    ...
    const actor = Actor.system('view-tracking-webhook');
    ...
    try {
      ...
      throw new BadRequestException(error instanceof Error ? error.message : 'Invalid view evidence');
    }

    try {
      application.markViewed(actor, correlationId, evidence, confidence);
    } catch (error) {
