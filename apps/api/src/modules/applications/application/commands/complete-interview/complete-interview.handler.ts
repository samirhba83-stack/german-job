...
import { CompleteInterviewCommand } from './complete-interview.command';
import {
...
export class CompleteInterviewHandler implements ICommandHandler<CompleteInterviewCommand> {
  ...
  async execute(command: CompleteInterviewCommand): Promise<ApplicationReadModel> {
    ...
    try {
      application.completeInterview(actor, correlationId, metadata);
    } catch (error) {
