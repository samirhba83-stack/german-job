...
export class CancelCampaignHandler implements ICommandHandler<CancelCampaignCommand> {
  ...
  async execute(command: CancelCampaignCommand): Promise<CampaignReadModel> {
    ...
    const reason = CampaignReason.create(command.reasonCode, command.reasonNote);

    try {
      campaign.cancel(actor, correlationId, reason);
    } catch (error) {
