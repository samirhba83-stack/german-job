...
function createPublishedJob(): Job {
  const job = Job.create(VALID_JOB_ID, VALID_COMPANY_ID, {
...
describe('CloseJobHandler', () => {
  ...
  beforeEach(() => {
    ...
    handler = new CloseJobHandler(jobRepository, companyRepository, eventBus as any);
  });

  it('closes a published job', async () => {
    const result = await handler.execute(new CloseJobCommand(VALID_JOB_ID, 'owner-1', UserRole.EMPLOYER));
  ...
  it('throws ForbiddenException for a non-owner, non-admin requester', async () => {
    await expect(
      handler.execute(new CloseJobCommand(VALID_JOB_ID, 'someone-else', UserRole.EMPLOYER)),
  ...
  it('throws NotFoundException when the job does not exist', async () => {
    ...
    await expect(
      handler.execute(new CloseJobCommand(VALID_JOB_ID, 'owner-1', UserRole.EMPLOYER)),
    ).rejects.toThrow(NotFoundException);
  });

  it('throws ConflictException when the job is not published', async () => {
    job.close();
