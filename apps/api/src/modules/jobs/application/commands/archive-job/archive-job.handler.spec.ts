...
import { EmploymentType, ContractType, CompanyIndustry, CompanySize, UserRole } from '@german-job-engine/shared-types';
import { ArchiveJobHandler } from './archive-job.handler';
...
describe('ArchiveJobHandler', () => {
  let jobRepository: jest.Mocked<JobRepository>;
  let companyRepository: jest.Mocked<CompanyRepository>;
  let eventBus: { publish: jest.Mock };
  let handler: ArchiveJobHandler;
  let job: Job;
  ...
  beforeEach(() => {
    ...
    handler = new ArchiveJobHandler(jobRepository, companyRepository, eventBus as any);
  });

  it('archives the job when the requester owns the company', async () => {
    const result = await handler.execute(new ArchiveJobCommand(VALID_JOB_ID, 'owner-1', UserRole.EMPLOYER));
  ...
  it('throws ForbiddenException for a non-owner, non-admin requester', async () => {
    await expect(
      handler.execute(new ArchiveJobCommand(VALID_JOB_ID, 'someone-else', UserRole.EMPLOYER)),
  ...
  it('throws NotFoundException when the job does not exist', async () => {
    ...
    await expect(
      handler.execute(new ArchiveJobCommand(VALID_JOB_ID, 'owner-1', UserRole.EMPLOYER)),
    ).rejects.toThrow(NotFoundException);
  });

  it('throws ConflictException when the job is already archived', async () => {
    job.archive();
